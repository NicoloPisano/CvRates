...
class User {
    ...
    public function __construct() {
        ...
        if(isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            $query = $db->query(sprintf("SELECT users.*, packages.*, T_Person.*
            FROM users 
            ...
            LEFT JOIN T_Person on users.user_id = T_Person.ID_Person
    ...
    public function settings($edit, $args) {
        ...
        switch ($edit) {
            ...
            //   /* validate country */
            //     if(is_empty($args['nationality'])) {
            //         throw new Exception(__("country variable is empty"));
            //     }

            
                // $check = $db->query(sprintf("SELECT * FROM D_Country WHERE ID = %s", secure($country, 'int') )) or _error(SQL_ERROR_THROWEN);
                // if($check->num_rows == 0) {
                //     throw new Exception(__("there is no id as the one selected"));
                // }
                ...
                /* update user */
                $db->query(sprintf("UPDATE T_Person 
                SET Firstname = %s, 
                Surname = %s, 
                Birthdate = %s,  
                Nationality = %s, 
                ...
                WHERE ID_Person = %s"
                , 
                secure($args['firstname']), secure($args['lastname']), secure($args['birth_date']), 
                secure($args['nationality']), secure($args['job']), secure($args['jobseeking']), secure($args['jobseekingcity']), 
                secure($args['seniority']),
            ...
            case 'education':
              ...
              /* validate startdate */
            if($args['edustart_month'] == "none" && $args['edustart_day'] == "none" && $args['edustart_year'] == "none") {
                $args['edustart_date'] = 'null';
            } else {
                if(!in_array($args['edustart_month'], range(1, 12))) {
                    throw new Exception(__("Please select a valid birth month"));
                }
                if(!in_array($args['edustart_day'], range(1, 31))) {
                    throw new Exception(__("Please select a valid birth day"));
                }
                if(!in_array($args['edustart_year'], range(1905, 2019))) {
                    throw new Exception(__("Please select a valid birth year"));
                }
                $args['edustart_date'] = $args['edustart_year'].'-'.$args['edustart_month'].'-'.$args['edustart_day'];
            }
                ...
                /* update user */
                $db->query(sprintf("UPDATE users SET user_edu_major = %s, user_edu_school = %s, user_edu_class = %s,  user_startdate = %s WHERE user_id = %s", secure($args['edu_major']), secure($args['edu_school']), secure($args['edu_class']), secure($args['edustart_date']),  secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
                break;
    ...
    public function sign_up($args) {
        ...
        $db->query(sprintf("INSERT INTO T_Person (Id_person, firstname, surname, birthdate) 
        Select user_id, 
        user_firstname, 
        user_lastname,
        user_birthdate
    ...
    public function get_countries() {
        global $db;
        $countries = array();
        $get_allcountries = $db->query("SELECT * FROM D_Country") or _error(SQL_ERROR_THROWEN);
        if($get_allcountries->num_rows > 0) {
            while($country = $get_allcountries->fetch_assoc()) {
                $countries[] = $country;
            }
        }
        return $countries;
    ...
    public function get_position() {
        global $db;
        $positions = array();
        $get_allposition = $db->query("SELECT * FROM D_Position") or _error(SQL_ERROR_THROWEN);
        if($get_allposition->num_rows > 0) {
            while($position = $get_allposition->fetch_assoc()) {
                $positions[] = $position;
            }
        }
        return $positions;
    ...
    public function get_seniority() {
        global $db;
        $seniorities = array();
        $get_allseniority = $db->query("SELECT * FROM D_SeniorityLevel") or _error(SQL_ERROR_THROWEN);
        if($get_allseniority->num_rows > 0) {
            while($seniority = $get_allseniority->fetch_assoc()) {
    ...
    public function get_university() {
        global $db;
        $universities = array();
        $get_alluniversities = $db->query("SELECT * FROM D_University") or _error(SQL_ERROR_THROWEN);
        if($get_alluniversities->num_rows > 0) {
            while($university = $get_alluniversities->fetch_assoc()) {
     ...
     * @return array
     */
    public function get_gradetype() {
        global $db;
        $grades = array();
        $get_allgrades = $db->query("SELECT * FROM D_EducationGrade") or _error(SQL_ERROR_THROWEN);
        if($get_allgrades->num_rows > 0) {
            while($grade = $get_allgrades->fetch_assoc()) {
                $grades[] = $grade;
            }
        }
        return $grades;
 ...
 /**
    ...
    * @return array
    */
   public function get_macrosubject() {
       global $db;
       $macrosubjects = array();
       $get_allmacrosubjects = $db->query("SELECT * FROM D_MacroSubject") or _error(SQL_ERROR_THROWEN);
       if($get_allmacrosubjects->num_rows > 0) {
           while($macrosubject = $get_allmacrosubjects->fetch_assoc()) {
               $macrosubjects[] = $macrosubject;
           }
       }
       return $macrosubjects;
   }
 ...
 /**
    ...
    */
    public function get_idiomas() {
        global $db;
        $idiomas = array();
        $get_alllanguages = $db->query("SELECT * FROM D_Language") or _error(SQL_ERROR_THROWEN);
        if($get_alllanguages->num_rows > 0) {
            while($idioma = $get_alllanguages->fetch_assoc()) {
                $idiomas[] = $idioma;
            }
        }
        return $idiomas;
 ...
 /**
    ...
    * @return array
    */
    public function get_completamientos() {
        global $db;
        $completamientos = array();
        $get_allcompletamientos = $db->query("SELECT * FROM S_CompletionTime") or _error(SQL_ERROR_THROWEN);
        if($get_allcompletamientos->num_rows > 0) {
            while($completamiento = $get_allcompletamientos->fetch_assoc()) {
                $completamientos[] = $completamiento;
            }
        }
        return $completamientos;
    }
     ...
     * 
     */
    // public function get_educations_ids() {
    //     global $db;
    //     $educations = array();
    //     $get_educations = $db->query(sprintf("SELECT ID FROM T_Education WHERE ID_Person = %s", secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
    //     if($get_educations->num_rows > 0) {
    //         while($education = $get_educations->fetch_assoc()) {
     ...
     */
    // public function get_educations($args = array()) {
    //     global $db, $system;
    //     /* initialize arguments */
    //     $user_id = !isset($args['user_id']) ? null : $args['user_id'];
    ...
    //             $get_educations = $db->query(sprintf("SELECT  
    //             T_Education.ID,
    ...
    //             WHERE T_Education.ID_Person = %s 

    //             ORDER BY T_Education.StartDate", 
    //             secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
        ...
        //     // $where_statement = ($this->_data['user_id'] == $user_id)? "": "'";
        //     $get_educations = $db->query(sprintf("SELECT  
        //         T_Education.ID,
        //         T_Education.ID_Person, 
        //         D_University.University,
        ...
        //         WHERE T_Education.ID_Person = %s", 
        //     secure($user_id, 'int') )) or _error(SQL_ERROR_THROWEN);
 ...
 /**
     ...
     * @return integer
     */
    public function create_education($university, $start_date, $end_date, $grade, $macrosubject, $country, $averagemark, $idioma, $thesis, $stduration, $completamiento) {
        global $db, $system, $date;
        /* check if events enabled */
        // if($this->_data['user_group'] >= 3 && !$system['events_enabled']) {
        //     throw new Exception(__("This feature has been disabled by the admin"));
        // }
        /* validate title */
        if(is_empty($university)) {
            throw new Exception(__("You must enter the field University"));
        }
        /* validate start & end dates */
        if(is_empty($start_date)) {
            throw new Exception(__("You have to enter the start date"));
        }
        // if(is_empty($end_date)) {
        //     throw new Exception(__("You have to enter the event end date"));
        // }
        ...
        /* insert new event */
        $db->query(sprintf("INSERT INTO T_Education (ID_Person, ID_University, Startdate, Enddate, ID_GradeType, ID_MacroSubject, ID_Country, AverageExams_Mark, ID_Language, Experimental_Thesis, Standard_Duration, Completion_Percentage) 
                            VALUES                      (%s,        %s,             %s,      %s,        %s,         %s,              %s,            %s,             %s,             %s,                 %s,                 %s)", 

        secure($this->_data['user_id']), secure($university), secure($start_date, 'datetime'), 
        secure($end_date, 'datetime'), secure($grade), secure($macrosubject), 
        secure($country), secure($averagemark), secure($idioma), secure($thesis), secure($stduration)
        , secure($completamiento))) or _error(SQL_ERROR_THROWEN);
        $education_id = $db->insert_id;
        /* join the group */
        // $this->connect("event-interest", $event_id);
        /* return event id */
        return $education_id;
